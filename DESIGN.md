...
**Generische Collections** ✅ demonstriert über eine mitkompilierte Java-Bibliothek (`examples/MiniList.java`): `MiniList<E>` mit internem `Object[]` + Wachstum; javac wendet Type-Erasure an, der Compiler sieht `Object`-Signaturen, der Aufrufer bekommt automatisch `checkcast` eingefügt (statisch/Laufzeit, s. §6a). Voll RC-verwaltet inkl. der beim Wachstum weggeworfenen Arrays. Der Weg für echtes `java.util` (statt Mini-Bibliothek) wäre `--patch-module`/bootclasspath-Kompilierung der Klassen. Offen: `HashMap`/`equals`-basierte Maps — bräuchten `String` als reguläres Objekt mit Vtable (aktuell immortal ohne `equals`/`hashCode`-Dispatch).
