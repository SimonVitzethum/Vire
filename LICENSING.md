...
| **cuda-oxide** — [`third_party/cuda-oxide/`](third_party/cuda-oxide/): the full upstream source is vendored under its own license (kept for reference/attribution of the `@gpu` design; `exclude`d from the build, never compiled) | **Apache License 2.0** | [`third_party/cuda-oxide/LICENSE`](third_party/cuda-oxide/LICENSE) |
