...
1. **Nicht schwerer als Python.** Einrückung statt Klammern, keine Semikolons,
   keine Typannotationen nötig. Was man aus Python kann, kann man hier.
2. **Statisch getypt durch Inferenz.** Kein Typ steht da, aber jeder ist bekannt
   (Hindley-Milner + lokale Bidirektionalität). Fehler zur Compilezeit, nicht zur
   Laufzeit.
...
Dateiendung `.lm`. Einstiegspunkt: die freie Funktion `main`.
...
fn greet(name):                   # Block; letzte Zeile ist der Wert
...
konfigurierbar) — wie Rust. `Int` ist der ergonomische Standard, die exakten
Breiten für Systemcode und FFI.
